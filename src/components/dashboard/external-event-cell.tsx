...
export function ExternalEventCell({
  eventId,
  eventName,
  field,
  value,
  ...
  eventId: string;
  eventName: string;
  /** Field to update: 'sold' or 'scanned' */
  field: "sold" | "scanned";
  value: number | null;
  ...
  async function save() {
    ...
    try {
      const res = await fetch(
        `/api/dashboard/external-ticket-events/${eventId}`,
        {
          method: "PATCH",
      ...
      const data = (await res.json().catch(() => ({}))) as {
        error?: string;
        event?: { sold?: number | null; scanned?: number | null };
      };
      if (!res.ok) {
      ...
      const next = data.event?.[field] ?? parsed;
