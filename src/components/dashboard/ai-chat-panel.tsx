...
export function AiChatPanel() {
  ...
  return (
    <div className="flex h-[min(560px,70vh)] flex-col border border-border bg-surface">
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        ...
        {pending && (
          <p className="animate-pulse-soft font-display text-sm tracking-[0.12em] text-text-dim">
            Denkt…
          </p>
        )}
      </div>
      ...
      <div className="border-t border-border p-3">
        <div className="mb-3 flex flex-wrap gap-1.5">
          {chatSuggestions.map((s) => (
            <button
              ...
              type="button"
              onClick={() => ask(s)}
              className="border border-border bg-bg px-2 py-1 text-[11px] text-text-muted transition-colors hover:border-accent hover:text-accent"
            >
              {s}
            </button>
          ))}
        ...
        >
          <input
            ...
            className="flex-1 border border-border bg-bg px-3 py-2 text-sm text-text outline-none placeholder:text-text-dim focus:border-accent"
          />
          <button
            type="submit"
            disabled={pending}
