...
export type TikTokAdsSyncResult = {
  ok: boolean;
  fetched: number;
  upserted: number;
  linked?: number;
...
/**
 * Read-only TikTok Marketing API sync → marketing_ads (ad-level reports).
 */
export async function syncTikTokAdsReadOnly(options?: {
  since?: Date | null;
  limit?: number;
  ...
  if (!(await ensureTikTokAdsAccessToken())) {
    return {
      ...
      fetched: 0,
      upserted: 0,
      error: "TIKTOK_ADS_ACCESS_TOKEN ontbreekt",
      notes,
    };
  }
  if (!hasDatabase()) {
  ...
  const adById = new Map((ads.ok ? ads.ads : []).map((ad) => [ad.adId, ad]));
  const db = getDb();
  let upserted = 0;
  const errors: string[] = [];
  ...
  for (const row of insights.rows) {
    ...
    try {
      ...
      await db
        ...
        .values({
          platform: "tiktok",
          adAccountId: account.account.advertiserId,
  ...
  await logIntegration({
    ...
    message: ok
      ? `TikTok ads sync: ${upserted}/${insights.rows.length} ads · ${account.account.name}`
      : errors[0] ?? "TikTok ads sync mislukt",
    detail: {
