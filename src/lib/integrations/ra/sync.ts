...
export async function syncResidentAdvisorReadOnly(): Promise<{
  ...
  if (!hasDatabase()) {
    return { ok: false, fetched: 0, upserted: 0, linked: 0, error: "DATABASE_URL ontbreekt" };
  }
  ...
  const batches = await Promise.all([
    listRaVenueEvents({ type: "LATEST", limit: 40 }),
    listRaVenueEvents({ type: "ARCHIVE", year, limit: 50 }),
    listRaVenueEvents({ type: "ARCHIVE", year: year - 1, limit: 50 }),
  ]);
  ...
  await logIntegration({
    ...
    level: "info",
    event: "sync.ok",
    message: `RA listings: ${upserted} events, ${linked} gekoppeld aan edities (${venue.venue.name})`,
    detail: { fetched: events.length, upserted, linked },
    throttleMs: 0,
  });
