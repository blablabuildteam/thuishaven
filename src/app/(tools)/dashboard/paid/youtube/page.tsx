...
export default async function PaidYouTubePage() {
  const hasCreds = googleAdsConfigured();
  const bundle = await loadMarketingAdsBundle({ platform: "youtube" }).catch(
    () => null,
  );
  const data = bundle ?? {
    ads: [],
  ...
  return (
    <div className="animate-fade-up">
      <SectionHeader
        ...
        description={
          hasCreds
            ? "Video- en Demand Gen-campagnes uit Google Ads · spend, views en clicks per campagne."
      ...
      {!hasCreds && data.ads.length === 0 ? (
        <div className="border border-border bg-surface p-5">
          <p className="text-sm font-medium">YouTube ads nog niet gekoppeld</p>
          <p className="mt-2 max-w-xl text-sm text-text-muted">
            Organic YouTube gebruikt de Data API. Paid data komt uit{" "}
            <span className="font-medium">Google Ads</span> (Video / Demand
            Gen). Check OAuth + Explorer access via{" "}
        ...
        <>
          <section className="mb-10 grid grid-cols-2 gap-x-4 gap-y-5 sm:grid-cols-3 sm:gap-x-8 sm:gap-y-6 lg:grid-cols-6">
            <Stat
              ...
              label="spend"
            />
            <Stat value={formatNumber(data.totals.reach)} label="video views" />
          ...
          <PaidAdsView
            bundle={data}
            emptyMessage="Nog geen YouTube ads (Video / Demand Gen) in deze periode."
          />
        </>
      )}
    </div>
